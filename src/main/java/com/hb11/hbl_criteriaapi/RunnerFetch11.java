package com.hb11.hbl_criteriaapi;
...
//		criteriaQuery.select(root).where(cb.greaterThan(root.get("mathGrade"),80)); // math notu 80 uzerinde olanlari getir
...
//		criteriaQuery.select(root).where(cb.lessThan(root.get("mathGrade"),95)); // 95 in altindaki butun ogrencileri getir
...
//		resultList4.forEach(System.out::println);
		...
		Long id=1L;
		// predicate bir belirtim yapiyoruz. and veya or diyorduk onun bir parcasi. 
		Predicate predicateForName=cb.equal(root.get("id"), id); // id 1 e esitse // obje ismi yanlislikla boyle kaldi
		Predicate predicateForMathGrade=cb.greaterThan(root.get("mathGrade"), 75); // 75 ten buyukse
		
		//Predicate predicateStd= cb.or(predicateForName,predicateForMathGrade); // yada
		Predicate predicateStd= cb.and(predicateForName,predicateForMathGrade); // burada hem hem olarak sadece 1 kisi dondu
		
		criteriaQuery.where(predicateStd); //icinde eger su obje sartlari saglaniyorsa
